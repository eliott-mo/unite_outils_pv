...
def calcul_extremaux(terrain, tr, echelle=5000):
    ...
    PERIM   = bnd.length
    MIN_D   = 0.05 * PERIM    # ecart mini entre deux points (euclidien)
    GAP_MAX = 0.30 * PERIM    # portion de perimetre toleree sans aucun point
    N_MAX   = 6
    ...
    # -- 1 et 2. Extremes cardinaux, repli sur un angle en cas de doublon -----
    sel = []
    for _cle in (lambda c: c[1], lambda c: c[0], lambda c: -c[1], lambda c: -c[0]):
        ...
        if _libre(_k, sel):
            sel.append(_k)
            continue
        _dispo = [i for i in range(len(cand)) if _libre(i, sel)]
        if _dispo:
            sel.append(max(_angles(_dispo) or _dispo, key=lambda i: _score(i, sel)))

    # -- 3. Combler les portions de perimetre depourvues de point -------------
    while len(sel) < N_MAX:
        ...
        if _gmax <= GAP_MAX:
            break
        # On cherche DANS le trou, et on y prend le meilleur angle plutot que le
        # point le plus proche du milieu : viser le milieu tombe souvent au
        # milieu d'un cote droit, sans interet pour decrire l'emprise.
        _deb    = _abs[_trous.index(_gmax)]
        _dispo  = [i for i in range(len(cand)) if _libre(i, sel)]
        _dedans = [i for i in _dispo if 0 < (cand[i][2] - _deb) % PERIM < _gmax]
        if not _dedans:
            break
        sel.append(max(_angles(_dedans) or _dedans, key=lambda i: _score(i, sel)))
    ...
    # Cout : traversee de l'emprise d'abord (une fleche ne doit pas couper le
    # terrain), alignement avec la direction sortante ensuite.
    _cout = np.zeros((len(_pts), len(_tmpl)))
    for _i, (_px, _py) in enumerate(_pts):
        _vx, _vy = _px - cx, _py - cy
        _nn = max(math.hypot(_vx, _vy), 1e-9)
        _vx, _vy = _vx / _nn, _vy / _nn
        for _t, ((_adx, _ady), (_ux, _uy)) in enumerate(_tmpl):
            _cout[_i, _t] = (round(_coupe((_px, _py), _adx, _ady), 1) * 1000.0
                             - (_vx * _ux + _vy * _uy))

    try:
        from scipy.optimize import linear_sum_assignment as _lsa
        _lig, _col = _lsa(_cout)
        _attr = dict(zip(_lig.tolist(), _col.tolist()))
    except Exception:
        _attr = {}
        _li, _lt = set(range(len(_pts))), set(range(len(_tmpl)))
        while _li:
            _i, _t = min(((i, t) for i in _li for t in _lt),
                         key=lambda p: _cout[p[0], p[1]])
            _attr[_i] = _t
            _li.discard(_i)
            _lt.discard(_t)
    ...
    if capteurs_clusters:
        for cluster in capteurs_clusters:
            if cluster is None or cluster.is_empty:
                continue
            draw_geom(ax, cluster, fc="none", ec="#000000", lw=2, alpha_fill=0, ls=_ZC_LS, zorder=5)
            draw_geom(ax, cluster, fc="none", ec="#1A6FBF", lw=0.8, alpha_fill=0, ls=_ZC_LS, zorder=5)
    else:
        # Aucun KML : zone capteurs standard (buffer négatif terrain)
        draw_geom(ax, capteurs, fc="none", ec="#000000", lw=2, alpha_fill=0, ls=_ZC_LS, zorder=5)
        draw_geom(ax, capteurs, fc="none", ec="#1A6FBF", lw=0.8, alpha_fill=0, ls=_ZC_LS, zorder=5)
